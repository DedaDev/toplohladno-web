...
export function useGetClue(game_id: number | null) {
  const { data = { nextHelpAt: 0, partialWord: [], keyboardClue: [] }, error, isLoading, mutate } = useSWR(game_id ? [`/get_clue`, game_id]: null, ([path, game_id]) => {
    return toplohladnoInstance.post(path, { game_id }).then((res) => res.data as { nextHelpAt: number, partialWord: string[], keyboardClue: string[] })
  })
